...
export const selftest: Fn = {
	name: "selftest",
	description:
		"Probe the fetch ladder and report which rungs are up. Live health check: fetches a tiny known URL through each egress path — direct (worker fetch), scrape (residential proxy / OpenWRT node), render_mac (Mac patchright node) — and reports whether the BROWSER binding (render_cf) is present. Every probe is guarded and hard time-bounded (default 8s each, override with timeout_ms) so a hung node can never hang selftest. Also reports `configured` — which credentials/bindings are present as booleans, WITHOUT calling the upstreams (no key spent, no rate limit touched). Returns JSON { rungs:{direct,scrape,render_mac,render_cf}, configured }. Never cached.",
	inputSchema: {
		type: "object",
	...
	run: async (env, args) => {
		...
		const [direct, scrape, render_mac] = await Promise.all([probeDirect(ms), probeScrape(env, ms), probeRenderMac(env, ms)]);
