...
describe("runTriage — infer signal wiring (#913)", () => {
	...
	it("#975 — a sensitive-sender message is NEVER persisted to the signal log, even when armed", async () => {
		const sensitive: TriageMsg = { id: "h1", from: "billing@clinic.example", subject: "Your explanation of benefits is ready", preview: "patient responsibility: $40" };
		const deps = mkDeps([...SAMPLE, sensitive]);
		const env = envWith({ MAIL_TRIAGE_ENABLED: "1", INFER_ARM_MAIL: "1" });
		env.AI = { run: vi.fn(async () => ({ data: [[0.1, 0.2]] })) };
		await runTriage(env, { cycle_id: "sig4" }, deps);
		expect(isSensitiveSender(sensitive.from!)).toBe(true);
		// Every non-sensitive SAMPLE message is logged, but the sensitive one contributes zero KV writes.
		expect(env.AI.run).toHaveBeenCalledTimes(SAMPLE.length);
		const signals = await readInferSignals(env, "mail");
