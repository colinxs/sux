...
//
// Split into scoped modules under fns/_util/ (#565) — byte/string primitives
// (_bytes.ts), the in-isolate fetch dedup cache (_fetch-cache.ts), and
// content-addressed store/CAS ref parsing (_store-ref.ts) — so an unrelated
// change to one no longer carries incidental risk to every fn that only needed
// e.g. `clamp` or `oj`. This file stays a thin re-export barrel so every
// existing `from "./_util"` import keeps working unchanged, plus the leftover
// helpers (URL validation, vault date formatting, the fetch/render/HTML-strip
// surface) that don't cleanly fit any of those three concerns.

import type { RtEnv } from "../registry";
import { smartFetch } from "../proxy";
import { errMsg, fromB64, isHttpUrl } from "./_util/_bytes";
import { FETCH_CACHE_MAX_TEXT, fetchCacheGet, fetchCacheSet, fetchDedupActive } from "./_util/_fetch-cache";
import { getBlob, storeRefUuid } from "./_util/_store-ref";

export * from "./_util/_bytes";
export * from "./_util/_fanout";
export * from "./_util/_fetch-cache";
export * from "./_util/_store-ref";
