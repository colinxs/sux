...
import { type ByteBudget, byteBudget, errMsg, FANOUT_BUDGET_MS, FANOUT_BYTE_BUDGET, fromB64, isHttpUrl, oj, pool, putBlob, toB64 } from "./_util";
...
type Opts = { method: string; pdf: boolean; gzip: boolean; ttlSeconds?: number; dropboxFolder?: string };
...
async function processUrl(env: any, rawUrl: unknown, opts: Opts, budget: ByteBudget, dropboxNames: Set<string>): Promise<UrlResult> {
	...
	try {
		...
		return { url, status: got.status, src_bytes: srcBytes, bytes: bytes.length, content_type: contentType, ref: ref.url, ...(applied.length ? { applied } : {}), ...(dropboxPath ? { dropbox_path: dropboxPath } : {}) };
	} catch (e) {
...
export const put: Fn = {
	...
	description:
		...
		"Returns a JSON array of { url, status, src_bytes, bytes, content_type, ref, applied?, dropbox_path? } (or { url, error } / { url, oversize } for a skipped one). Composes batch_fetch (download), pdf (convert) and the store — the bulk sibling of `store`.",
	inputSchema: {
		type: "object",
	...
	run: async (env, args) => {
		...
		const opts: Opts = { method: String(args?.method ?? "GET").toUpperCase(), pdf: args?.pdf === true, gzip: args?.gzip === true, ttlSeconds, dropboxFolder };
		...
		const runBatch = async (): Promise<UrlResult[]> => {
			...
			const raw = await pool(urls, CONCURRENCY, (rawUrl) => processUrl(env, rawUrl, opts, budget, dropboxNames), deadline);
		...
		const payload = { urls, method: opts.method, pdf: opts.pdf, gzip: opts.gzip, ttl_seconds: opts.ttlSeconds, dropbox: opts.dropboxFolder ?? null };
		const preview = { action: "put batch", url_count: urls.length, ttl_seconds: opts.ttlSeconds ?? "permanent", ...(opts.pdf || opts.gzip ? { transforms: [...(opts.pdf ? ["pdf"] : []), ...(opts.gzip ? ["gzip"] : [])] } : {}), ...(opts.dropboxFolder !== undefined ? { dropbox: opts.dropboxFolder } : {}), sample_urls: urls.slice(0, 5).map((u) => (typeof u === "string" ? u : "")) };
