...
export const agenda: Fn = {
	...
	cacheable: false,
	description:
		"Figure out what needs you — scan your mail + calendar + Lunch Money (when LUNCHMONEY_API_KEY is set) for 'drops about to happen' (a prescription lapsing, a payment failing, a medical message, an unanswered personal note, an upcoming appointment, a bill due soon, an unusual charge, a low account balance), record a REVERSIBLE Todoist-task proposal for each (approve/snooze/reject via the `proposals` verb — nothing acts until you say so), and compose ONE calm digest of it all. action:'run' (default) runs a cycle; dry_run:true detects + shows the digest but records/sends nothing. DORMANT unless AGENDA_ENABLED; the digest is appended to today's Daily note, and additionally EMAILED to you when AGENDA_EMAIL is also set. Detectors are cheap rules (no model sorts your mail or moves money); each source degrades independently. This is the proactive half of the agent — cut the noise, catch the drop.",
	inputSchema: {
		type: "object",
