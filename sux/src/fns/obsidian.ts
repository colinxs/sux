...
async function ghJson(env: any, url: string, init?: { method?: string; body?: string }): Promise<{ status: number; json: any }> {
	...
	// contents:write) over the ambient GITHUB_TOKEN — set as a CALLER header so
	// smartFetch's caller-wins merge applies it to vault traffic ONLY (never other
	// fetches). Empty (a no-op) until the SUX_BOT_* App secrets are configured.
