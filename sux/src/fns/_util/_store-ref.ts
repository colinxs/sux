// Content-addressed blob store (shared with the `store` fn) + /s/<uuid> CAS ref
// parsing. Split out of _util.ts (#565) so a change to the store/KV plumbing
// can't carry incidental risk to fns that only need e.g. `clamp` or `oj`.

import type { RtEnv } from "../../registry";
import { maybeCompress, maybeDecompress } from "../_gzip";
import { errMsg, isHttpUrl, oj, toB64 } from "./_bytes";
import { FANOUT_STORE_TTL_S } from "./_fanout";
