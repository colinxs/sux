	...
	run: async (env, args) => {
		...
		try {
			...
			if (op === "get") {
				...
				if (isTexty(type)) return ok(oj({ key: r2key, size: obj.size, content_type: type, text: await obj.text() }));
				return ok(oj({ key: r2key, size: obj.size, content_type: type, base64: toB64(new Uint8Array(await obj.arrayBuffer())) }));
			}
