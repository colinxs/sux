...
def is_private_ip(ip):
    ...
    # through. This check runs after navigation against the address Chromium
    # actually connected to (see render_on's server_addr() check below), so it
    # can't be raced by re-resolving the name a second time.
    try:
...
async def render_on(context, spec, interactive=False):
    ...
    # Every navigation response (the initial request AND each hop of a
    # redirect chain) lands here; checked below against the address Chromium
    # actually connected to, not a separately re-resolved name.
    nav_responses = []
    page.on("response", lambda r: nav_responses.append(r) if r.request.is_navigation_request() else None)
    try:
        ...
        # Check every navigation seen so far (initial goto, redirect hops, and
        # any client-side navigation during the waits above) against the
        # address actually connected to — closes the DNS-rebinding gap the
        # Worker's hostname-text-only isBlockedTarget can't (see is_private_ip).
        for r in nav_responses:
