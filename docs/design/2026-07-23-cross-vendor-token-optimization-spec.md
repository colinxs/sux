...
## 9. Amendments (2026-07-24) — two premises changed after this spec was written

### 9.1 §2.1's four classes conflict with the merged sensitivity lattice — resolve one-way

`sux#1457` landed a real `Sensitivity` type (`sux/src/fabric/sensitivity.ts`) after this spec
was drafted. This spec assumed that type had to be built; it now exists. But the two models are
not interchangeable and the difference is load-bearing:

- **§2.1 is a total order** — `secret` > `personal` > `internal` > `public`. One bucket per value.
- **The lattice is a deliberately PARTIAL order over an unranked tag SET** — `phi`, `legal`,
  `embarrassment`, `secrets`, `financial`. Unranked because they disqualify destinations by
  *different mechanisms*: `legal` by **category** (third-party disclosure can waive privilege
  regardless of that party's posture), `phi`/`embarrassment` by **counterparty attribute** (who
  the operator is and what they agreed to). A public court filing is legally fine anywhere but
  embarrassing; a routine lab value is PHI but not embarrassing. Neither dominates.

Collapsing both into a single `personal` bucket is exactly the flattening the lattice exists to
prevent, and it would permit the disclosures that matter most.

**Resolution: the lattice is the record of truth; the routing class is DERIVED from it, one-way,
never the reverse.** Derivation is lossy, so it must never round-trip.

| routing class | derived predicate over the tag set |
|---|---|
| `secret` | intersects `{secrets}` |
| `personal` | intersects `{phi, legal, embarrassment, financial}` |
| `internal` | empty set, but provenance is first-party operational data |
| `public` | empty set from a public-provenance source |

The fail direction comes free: `classify()` resolves absent/malformed/partly-unrecognized input
to `MAX_SENSITIVITY`, so an unclassified value can never derive `public` by accident. §2.3's gate
should consult the tag set directly for the category-style refusals (`legal`) rather than the
derived class, which cannot express them.

`join()` also supersedes any per-call re-classification: a value derived from several sources
carries the **union** of their tags, so a summary of a PHI document is still PHI. Without that,
one transformation launders the classification and every downstream gate reasons about a value
whose provenance has been discarded.

### 9.2 §6 prompt consolidation — match the writer model to the runner, and use the vendor tooling

Two facts this spec did not account for:

1. **Subscription tooling is available and budget is not the binding constraint.** Both the
   Anthropic Console (prompt *generator* for a cold start, prompt *improver* for an existing
   prompt) and OpenAI's own prompt tooling are on hand, and further subscriptions can be bought
   if a workstream needs one. Do not design around tool scarcity.
2. **The writer model must match the runner model.** A prompt tuned by Claude for a prompt that
   Claude will run is a matched pair; the same prompt tuned by Claude and then executed by a 3B
   model is not.

That second point decides the method for the 21 `*_SYSTEM` constants, and it cuts against the
obvious move:

- **`llm()`'s primary runner is Workers-AI `llama-3.2-3b`** (OpenAI is the fallback in `ai.ts`).
  So for those call sites, **neither Console's improver nor OpenAI's is the matched writer.**
  Rewriting them in a Claude Console is the intuitive step and the wrong one.
- For a call site with a real eval set, **DSPy** optimizes against the actual runner and is the
  correct tool. Where there is no eval set, build the smallest one first — a handful of
  input/expected pairs beats an unmeasured rewrite by a stronger model.
- The vendor consoles ARE the right tool for prompts whose runner is that vendor's model — the
  cross-vendor verify path (§4) and any Claude/GPT-targeted CI prompt.

Practical consequence for §6: consolidating the 21 constants into one `prompts/` module is still
correct and is a prerequisite, because it makes A/B-ing a format change across all sites
possible. But the consolidation must not be conflated with *rewriting* them. Land the module
first, then optimize per call site with the tool matched to that site's runner.
