...
| `GITHUB_TOKEN` | **three consumers, one token**: vault git writes (Contents API, `obsidian`/`citation` — but see `SUX_BOT_*` below, which now takes precedence for vault writes when set); self-improve PR creation (gated by `SELF_IMPROVE_PR`); `smartFetch`'s GitHub-host rate lift (`github-auth.ts`, anonymous 60/hr → authenticated 5000/hr, read-only need). Least-privilege intent: a fine-grained PAT scoped to just the vault repo(s), not classic all-repo — see [[secrets\|docs/secrets.md]]. Rotate on suspicion. | ✅ set |
| `SUX_BOT_APP_ID`, `SUX_BOT_PRIVATE_KEY`, `SUX_BOT_INSTALLATION_ID` | **vault write-auth as the `suxbot` org App** (`github-app.ts`). All three set ⇒ `obsidian`/`citation`/`ingest` mint a short-lived org installation token (suxbot is installed org-wide on SuxOS with `contents:write`, install id `146421817`) instead of `GITHUB_TOKEN`, keeping the write credential org-scoped and off any personal PAT's rate ceiling. Opt-in: any unset ⇒ silent fallback to `GITHUB_TOKEN`. Private key is the App PEM (PKCS#1 or #8). Pair with `OBSIDIAN_VAULT_REPO='SuxOS/vault'`. | ⏳ pending (arm to write the org vault) |
