...
Four implementations behind the `obsidian` fn's `backend` dispatch, covering the stateful notes namespace reached via the `vault_*` verbs on the one `/mcp` connector. **`git`** talks to GitHub REST against `colinxs/obsidian-vault` @ `main` — always-on, no box or LAN required, and the correctness floor of the whole system: every write lands as a revertible commit. **`vpc`** is live headless Obsidian reached over Workers VPC plus `cloudflared` on the home box — the primary cloud path once built, giving the cloud vault (the `vault_*` verbs on `/mcp`) live-vault capability without depending on a box being awake at request time. **`obsidian`** is the Mac-live backend reached over Tailscale, used when a tailnet desktop is present. **`local-git`** is a direct filesystem clone, the local-ops counterpart to `git`.
