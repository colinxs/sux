...
Verb integration: **capture** an exchange → note with provenance (contact, timestamps, thread id); **retrieve** gains a messages source for "what did X and I decide"; **remember** extracts the durable fact. Contact resolution rides jmap's `ContactCard/query` (RFC 9610 JSCard — not the legacy `Contact`) rather than a second address book.
...
**Phase 2 — jmap: email + contacts (ContactCard) + notes** *(calendar/tasks shipped separately via CalDAV, see §jmap)*
...
5. ~~**Fastmail calendars/contacts confirmation.**~~ **Verified 2026-07-08** via the claude.ai connector (JMAP-backed): the account serves 23 calendars (Personal, Family, Bills, a leftover Todoist feed, …) and a populated address book (contact groups imported 2026-07-08). **[Shipped correction 2026-07-11:** "contacts = jmap" held (via `ContactCard`, RFC 9610), but **"calendar = jmap" did NOT** — Fastmail advertises no JMAP calendars URN, so calendar/tasks shipped over **CalDAV** (`_caldav.ts`), not `jmap`.]** Residual check at token time: the Session object's capability URNs (`…:contacts` for contacts; no `…:calendars` is advertised), per jmap.md §16 feature-detection.
