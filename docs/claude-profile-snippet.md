...
- **Text / AI:** `classify` (zero-shot labels), `entities` (regex NER), `redact` (scrub PII), `voice` (restyle into a tone or a learned profile), `preferences` (learn + persist that writing voice for `voice` to reuse).
