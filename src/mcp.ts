...
export const TOOL_DESCRIPTION_OVERRIDES: Record<string, string> = {
	// Bake scoping guidance + the lens name→ID mapping into the tool description so
	// the model can "lens" a search without a separate tool.
	kagi_search_fetch:
		"Fetch web results for a query using Kagi. Results are numbered so a user can refer to one by number. " +
		"Scope results with optional args: include_domains / exclude_domains (restrict or omit sites), " +
		"time_relative ('day'|'week'|'month') or after/before (recency), file_type (e.g. 'pdf'), " +
		"workflow ('news'|'videos'|'podcasts'|'images'; default 'search'), and set extract_count>0 to inline full page content. " +
		"To scope to a curated source set, pass lens_id — Kagi lenses by name: Academic=2, Forums=1, Programming=15, News360=29, Recipes=120, Small Web=107 " +
