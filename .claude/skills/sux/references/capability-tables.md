...
| Monarch Money — READ-ONLY personal finance (accounts, balances, transactions, budgets, cashflow, categories, holdings) | `monarch` (sux NEVER moves money — no transfer/trade op exists and the raw `graphql` escape refuses mutations; paste your session token once at the operator-gated `/monarch/connect`, stored read-only in KV — absent/stale → not_configured) |
